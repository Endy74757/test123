pipeline {
    ...
    environment {
        ...
        TARGET_URL = 'http://34.2.23.55:8888' 
    ...
    stages {
        ...
        stage('2. Run OWASP ZAP Automation Scan (Docker One-Shot)') {
            steps {
                script {
                    ...
                    sh """
                    docker run --rm -u 0 -v \${PWD}:/zap/wrk/:rw \\
                    -e TARGET_URL=${env.TARGET_URL} zaproxy/zap-stable  \\
        ...
        stage('3. Publish Report') {
            steps {
                ...
                // archiveArtifacts artifacts: 'reports/ZAP_Report_Target.html', fingerprint: true
                tar file: 'zap-scan-report.tar.gz', dir: 'reports', compress: true, archive: true
            }
        }
    }
}